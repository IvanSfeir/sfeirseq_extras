...
# Build dataframe
predictions = pd.read_csv(parent_path+"test_predictions", delimiter="\\t", \
	names=["sentence_idx","source","target","prediction"])


def preprocess():
	# Saves predictions in a CoNLL-friendly format in parent path
	# Data is ready for conlleval.pl script
	...
	df_conll_format.to_csv(parent_path+"test_predictions_for_eval", sep=' ', index=False, header=False)
